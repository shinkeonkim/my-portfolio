import type { Project } from '@/types'

const IMG = '/images/projects/lotteng'

export const lotteng: Project = {
  slug: 'lotteng',
  name: 'Lotteng (롯땡)',
  subtitle: '롯데 계열사 마감 세일 통합 O2O 플랫폼',
  period: { start: '2020-09', end: '2020-10' },
  scale: 'hackathon',
  status: 'completed',
  roles: ['백엔드', '인프라'],
  team: { size: 8, lead: false },
  stack: ['Django', 'JavaScript', 'AWS', 'Nginx'],
  oneLiner:
    'LOTTENG = LOTTE + N(&) + G(Great deal): 롯데 계열사의 마감 세일 정보를 통합하고, 오프라인 픽업과 환경 보호를 연계하는 O2O 플랫폼.',
  description:
    '<p>LOTTENG(롯땡, 롯데 + 땡처리) 은 롯데 계열사의 마감 세일 정보를 통합한 O2O 플랫폼입니다.</p>' +
    '<p>롯데마트, 롯데백화점, 세븐일레븐, 롭스, 롯데호텔, 롯데리아를 대상으로 온라인 결제와 오프라인 매장 픽업을 연계합니다.</p>' +
    '<p>코로나19 이후 언택트 소비 급증으로 인한 오프라인 매출 감소와 택배 일회용품 증가, 두 가지 문제를 풀고자 했습니다.</p>' +
    '<p>환경 세일 카테고리를 별도로 두어 소비자가 할인을 받으면서 환경 보호에 동참하도록 설계했습니다.</p>' +
    '<p>8인 팀의 백엔드 + 인프라를 담당했습니다. Lotte x Likelion 연계 해커톤(2020 멋쟁이사자처럼 8기) 동상 수상.</p>',
  features: [
    {
      title: '지도 기반 매장 표시',
      content: ['잠실 근처 롯데 계열사 위치를 표시하고, 계열사 다중 선택과 토글 해제로 필터링 (초기값 ALL)'],
    },
    {
      title: '상품 카드',
      content: [
        '정가, 할인가, 자동 계산한 할인율, 마감 기한, 재고, 환경 세일 배지를 한 카드에 표시',
        '검색과 반응형 레이아웃 (웹 4열 / 모바일 1열)',
      ],
    },
    {
      title: '정렬',
      content: ['추천(최신순, AI 맞춤 추천 도입 염두), 저가순, 고가순, 잠실역 기준 거리순'],
    },
    {
      title: '결제 흐름',
      content: [
        '장바구니 → AJAX 결제 → 인증번호 발급',
        '난수로 발급한 뒤 매장 내 중복을 검사하고, 충돌하면 재발급',
      ],
    },
    {
      title: '회원',
      content: ['일반·구글 소셜 로그인, 마이페이지, 회원 정보 수정, 주문 내역, 비밀번호 변경'],
    },
    {
      title: 'AJAX 비동기',
      content: [
        '필터 / 장바구니 / 검색 / 인증번호가 새로고침 없이 동작',
      ],
    },
    {
      title: '반응형',
      content: ['모바일 side nav-bar, 비회원이 쓸 수 없는 버튼만 비활성화하는 UX'],
    },
  ],
  challenges: [
    {
      title: '인증번호 중복 발급 방지: 난수 + 매장 내 중복 검사',
      tags: ['Order code', 'Idempotency'],
      problem:
        '<p>오프라인 픽업 인증번호가 같은 매장에서 중복되면 서로 다른 주문을 구분할 수 없었습니다.</p>',
      approach:
        '<ol>' +
        '<li>난수로 인증번호를 생성했습니다.</li>' +
        '<li>같은 매장의 활성 주문과 번호가 겹치는지 확인했습니다.</li>' +
        '<li>중복이면 새 번호를 생성했습니다.</li>' +
        '</ol>',
      result:
        '<p>운영 중 인증번호 충돌 사례 없이 시연을 완료했습니다.</p>',
    },
    {
      title: '해커톤 일정 안에서 데모 안정성 우선의 인프라',
      tags: ['AWS EC2', 'Nginx', 'Gunicorn'],
      problem:
        '<p>제한된 해커톤 일정 안에 서비스를 구현하면서 시연용 인프라도 직접 운영해야 했습니다.</p>',
      approach:
        '<ul>' +
        '<li>AWS EC2, Nginx, Gunicorn을 단일 인스턴스에 구성했습니다.</li>' +
        '<li>확장성보다 발표 당일의 단순성과 안정성을 우선했습니다.</li>' +
        '</ul>',
      result:
        '<p>시연 중 다운 없이 끝까지 운영했고, Lotte x Likelion 연계 해커톤 동상을 수상했습니다.</p>',
    },
    {
      title: '주요 인터랙션을 AJAX 로 비동기화',
      tags: ['AJAX', 'UX'],
      problem:
        '<p>검색·필터, 장바구니 추가·삭제, 인증번호 발급 같은 주요 액션마다 페이지가 새로고침돼 흐름이 끊겼습니다.</p>',
      approach:
        '<ul>' +
        '<li>필터, 장바구니, 검색, 인증번호 발급을 AJAX 요청으로 전환했습니다.</li>' +
        '<li>비회원은 사용할 수 없는 액션만 비활성화했습니다.</li>' +
        '</ul>',
      result:
        '<p>페이지를 다시 불러오지 않고 주요 작업을 처리했고, 회원 상태에 따른 안내도 명확해졌습니다.</p>',
    },
  ],
  contributions: [
    {
      title: '백엔드',
      summary: 'Django + AJAX 인터랙션 + 인증번호 발급 로직.',
    },
    {
      title: '인프라',
      summary: 'AWS EC2(Ubuntu 18.04 LTS) + Nginx + Gunicorn.',
    },
    {
      title: '도메인 로직',
      summary: '롯데 계열사 통합 모델링 + 환경 세일 배지 + 거리순 정렬 (8인 팀 협업).',
    },
  ],
  links: [
    { label: 'GitHub', url: 'https://github.com/hyeoneedyou/lotteng', type: 'github' },
    { label: 'Instagram', url: 'https://www.instagram.com/lotteng_official/', type: 'other' },
    {
      label: '해커톤 발표 게시물',
      url: 'https://www.instagram.com/p/CHwRaZRBxl5/',
      type: 'article',
    },
  ],
  award: 'Lotte x Likelion 연계 해커톤 동상 (2020)',
  hero: `${IMG}/01-logo.png`,
  media: [
    {
      type: 'image',
      url: `${IMG}/01-logo.png`,
      caption: '메인 화면: 서비스 로고 LOTTENG = Lotte + N(&) + G(Great deal) + 검색 + 지도 기반 잠실 근처 롯데 계열사 위치',
    },
    {
      type: 'image',
      url: `${IMG}/02-main.png`,
      caption: '추천 상품 목록: 정가/할인가/할인율(자동 계산)/마감 기한/재고 + 환경 세일 배지',
    },
    {
      type: 'image',
      url: `${IMG}/03.png`,
      caption: '상품 상세: LOTTE 호텔 객실 + 개수 조정 + 장바구니 담기',
    },
    {
      type: 'image',
      url: `${IMG}/04-map.png`,
      caption: '장바구니: 담은 상품 + 개수 조정 + 결제 예정 금액',
    },
    {
      type: 'image',
      url: `${IMG}/05.png`,
      caption: '결제: 결제 수단 선택 + 픽업장소 + 결제 금액 안내',
    },
    {
      type: 'image',
      url: `${IMG}/06-cart.png`,
      caption: '결제 완료 → 인증번호 발급 (난수 + 매장 내 중복 검사로 충돌 시 재발급)',
    },
    {
      type: 'image',
      url: `${IMG}/07-payment.png`,
      caption: '개발팀 소개: 2020 멋쟁이사자처럼 8기 8인 팀 (footer)',
    },
  ],
}
